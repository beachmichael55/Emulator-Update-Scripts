...
set repo=xenia-project/release-builds-windows
...
echo Downloading...
:: Download release
for /f "tokens=2 delims= " %%a in ('curl -s %link% ^| findstr /l "browser_download_url" ^| findstr /v /r "pdbs|uibase|src" ^| findstr /l "master.zip"') do (set dl=%%a)
