...
set repo=cemu-project/Cemu
set link=https://github.com/%repo%/releases
...
:CheckNewVersion
:: Get latest version
for /f "delims=" %%a in ('powershell -command "& { (Invoke-WebRequest '%link%' -UseBasicParsing).Content -match 'v(\d.\d-\d+)' | Out-Null; $matches[1] }"') do set ver=%%a
...
mkdir "%temparchive%"
:: Download release
set Durl=https://github.com/cemu-project/Cemu/releases/download/v%ver%/cemu-%ver%-windows-x64.zip
...
for /D %%I in ("%~dp0temp\Cemu*") do (set temparchiveExpand=%%~I)
ROBOCOPY %temparchiveExpand% %WorkDir% /E >nul 2>&1
:: ::ove extracted temp folder
rd "%temparchive%" /s /q >nul 2>&1
:: Delete temporary files
del /f /q %dlfile% >nul 2>&1
del /f /q %DownloadErrorFile% >nul 2>&1
:: Update VERSION file
