...
set repo=PCSX2/pcsx2
set link=https://api.github.com/repos/%repo%/releases
...
echo Downloading...
:: Download release
for /f "tokens=2 delims= " %%a in ('curl -s %link% ^| findstr /l "browser_download_url" ^| findstr /v /r "pdbs|uibase|src" ^| findstr /l "Qt.7z"') do (set dl=%%a)
