...
set link=https://www.ppsspp.org/download
...
for /f "delims=" %%a in ('powershell -command "& { (Invoke-WebRequest '%link%' -UseBasicParsing).Content -match '(\d+\.\d+\.\d+)' | Out-Null; $matches[1] }"') do set ver=%%a

:: Replace periods with underscores. Used In DOWNLOAD subroutine
set "string=%ver%"
set "result=!string:.=_!"
...
:: Download release
set Durl=https://www.ppsspp.org/files/%result%/ppsspp_win.zip
...
where 7z > nul 2>&1
if %errorlevel% equ 0 (
	7z x %dlfile% -aoa >nul 2>&1
) else (
	if defined Seven_Zip (
		%Seven_Zip% x %dlfile% -aoa >nul 2>&1
	) if defined WinRAR (
	%WinRAR% x -y -o+ %dlfile% >nul 2>&1
	)
)
:: Delete temporary files
del /f /q %dlfile% >nul 2>&1
