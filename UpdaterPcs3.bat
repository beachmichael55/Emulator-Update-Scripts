...
set dlfile=%temp%\%ScriptName%.7z
set "lastverpath=%OriginalDir%Versions\%ScriptName%_VERSION.txt"
set repo=RPCS3/rpcs3-binaries-win
...
:CheckNewVersion
:: Get latest version
for /f "delims=" %%a in ('powershell -command "& { (Invoke-WebRequest '%link%' -UseBasicParsing).Content -match 'rpcs3-v(\d.\d.\d+-\d+)' | Out-Null; $matches[1] }"') do set ver=%%a
...
:: Download release
for /f "tokens=2 delims= " %%a in ('curl -s %link% ^| findstr /l "browser_download_url" ^| findstr /l ".7z" ^| findstr /v /l ".sha256"') do (set dl=%%a)
