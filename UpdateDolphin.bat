...
set temparchive=%~dp0temp
set "lastverpath=%OriginalDir%Versions\%ScriptName%_VERSION.txt"
set link=https://dolphin-emu.org/download/
...
:CheckNewVersion
:: Get latest version
:: Invoke PowerShell to scrape the version number from the website
for /f "delims=" %%a in ('powershell -command "& { (Invoke-WebRequest '%link%' -UseBasicParsing).Content -match 'releases/(\d+\d+\d+\d+)/dolphin-(\d+\d+\d+\d+)-x64.7z' | Out-Null; $matches[1] }"') do set ver=%%a

:CheckIfNew
:: Checks if new one is available
if %ver% GTR %oldver% (
	GOTO MENU
	)
...
:: make extracted temp folder
mkdir "%temparchive%" 2 >nul 2>&1
:: Download release
set Durl=https://dl.dolphin-emu.org/releases/%ver%/dolphin-%ver%-x64.7z
if not exist %dlfile% (powershell -command "& {Invoke-WebRequest -Uri %Durl% -OutFile %dlfile%}") >> %DownloadErrorFile% 2>&1
...
:: copy updated files to Home from the extract temp folder
ROBOCOPY %temparchive%\Dolphin-x64 %~dp0 /E 2 >nul 2>&1
:: Remove extracted temp folder
rd "%temparchive%" /s /q 2 >nul 2>&1
:: Delete temporary files
del /f /q %dlfile% >nul 2>&1
del /f /q %DownloadErrorFile% >nul 2>&1
...
:: Deletes temporary version file
del /f /q %versionFile% >nul 2>&1
endlocal
exit