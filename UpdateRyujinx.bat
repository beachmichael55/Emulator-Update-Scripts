...
set dlfile=%temp%\ryujinx.7z
set "lastverpath=%OriginalDir%Versions\Ryujinx_VERSION.txt"
set temparchive=%~dp0temp
set repo=Ryujinx/release-channel-master
...
mkdir "%temparchive%"
:: Download release
for /f "tokens=2 delims= " %%a in ('curl -s %link% ^| findstr /l "browser_download_url" ^| findstr /v /r "pdbs|uibase|src" ^| findstr /l "win_x64.zip"') do (set dl=%%a)
...
ROBOCOPY %temparchive%\publish %WorkDir% /E >nul 2>&1
